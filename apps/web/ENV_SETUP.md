...
Create a `.env.local` file in `apps/web/` directory with the following
variables:
...
4. **Set NEXT_PUBLIC_CHAIN=celo** for mainnet, or leave unset (auto-detects from
   VERCEL_ENV)
...
Contract addresses are hardcoded in `src/lib/constants.ts` and automatically
selected based on the chain. No environment variables needed for contract
addresses.
